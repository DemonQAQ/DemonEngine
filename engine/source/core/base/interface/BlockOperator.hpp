...
namespace base
{
    ...
    public:
        ...
        virtual void writeToBlock(const std::shared_ptr<Metadata> &metadata, std::shared_ptr<io::YamlConfiguration> &yml) = 0;

        // 子类需要实现此方法，负责从指定yaml中读取数据存入metadata
        virtual void readFromBlock(const std::shared_ptr<Metadata> &metadata, std::shared_ptr<io::YamlConfiguration> &yml) = 0;
